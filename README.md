...
## Private ISP dashboards

`speedtest.levizitting.com` and `smokeping.levizitting.com` resolve through VPN split-DNS to the LZ cluster. Use HTTPS port `8443` and the existing private CA trust. The public edge excludes these hostnames from HTTP and TLS passthrough, and does not publish port 8443. The cluster also keeps their routes off ports 80/443, so private DNS is not the access-control boundary.
